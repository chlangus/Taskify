...
- 팀원 모두 테일윈드 사용 경험이 없었고, nextjs(사용하지 않게 되었지만)의 앱 라우팅과 호환성이 좋아서 scss와 고민 중에 테일윈드를 도입하게 되었다.
- css 파일이 별도로 필요하지 않아서 파일 개수가 줄어들고, 클래스 네이밍을 고민하지 않아도 되었다.
...
- 자동완성 기능이 주어져 데이터의 어떤 부분을 사용해야 하는지 다시 한번 확인해야 하는 번거로움을 줄여줄 수 있었다.
...
### 2️⃣ 구성원/초대 목록 첫 로딩 or 변경될 때 페이지네이션 동작 중 깜빡거리는 문제
### 💡 해결 
